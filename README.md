...
编辑器样式由 `source/admin/admin-ui.js` 为控件添加明确标记，再由 `admin-theme.css` 应用；修改时同时递增 `content/index.html` 的资源版本。正文工具栏只识别 `ToolbarContainer`，不能把包含 `Toolbar` 的所有下拉容器都当作工具栏；日期快捷按钮按 `data-testid` 识别，不依赖可见文字与 `aria-label` 完全相同。字段与网格槽可能是同一个元素，重置样式不能清掉槽的间距。

首页的最新文章与置顶推荐是同级独立入口，分别展示各自摘要和阅读链接；同一篇同时满足两种条件时只展示一次。最新顺序仍依据发布日期，置顶不改变它。
...
**弹窗进入管理页不等于前台登录成功。** 必须等前台评论区出现已登录账户，才能确认回传完成。评论区增加了不接管认证的状态提示，区分等待、回传缺失和实际连接成功；观察过程不保存、输出或转交令牌。不要用重复刷新登录弹窗来代替检查。

2026-09-09 检查时，生产站点与 Waline 登录页没有 COOP/COEP 阻断头，新域名请求评论 API 也未被拒绝，不能把这次问题直接归因于跨域白名单。Waline 后台 `@waline/admin@0.34.2` 恢复临时会话时存在 `TOKEN`/`token` 存储键大小写不一致的条件性缺陷；是否命中必须核对实际回传，不能仅凭登录成功截图下结论。后台服务独立部署，在本仓库改前端提示并不等于已经修复后台认证。
